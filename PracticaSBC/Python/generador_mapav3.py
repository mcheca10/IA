...
# --- CONFIGURACIÓN ---
NOMBRE_ARCHIVO_CLP = "instancias_v6.clp"
NOMBRE_IMAGEN = "mapa_v6.png"
NUM_VIVIENDAS = 100
NUM_SERVICIOS = 40
NUM_SOLICITANTES = 0 
CIUDAD_SIZE = 5000

print(f"--- GENERADOR DE INSTANCIAS (COMPATIBLE ONTOLOGIA V5/V6 LIMPIA) ---")
...
for i in range(NUM_VIVIENDAS):
    ...
    v = {
        ...
        "parking": random.choice(["SI", "NO"]) # Añadido parking
    }
    viviendas.append(v)
...
# 3. GENERAR SOLICITANTES (Adaptados a V5/V6)
...
for i in range(NUM_SOLICITANTES):
    ...
    # Presupuesto ajustado al precio de una casa random
    presupuesto_obj = casa_target['precio']
    
    # Superficie deseada (un poco menos de lo que tiene la casa target para que encaje)
    ...
    if clase == "Familia":
        ...
        # Eliminados num_hijos y colegio (no se usan)
    elif clase == "Estudiantes":
        num_personas = random.randint(2, 4)
        # Eliminado necesita_fiesta (no se usa)
    elif clase == "Pareja":
        num_personas = 2
        # Eliminado plan_familia (no se usa)
    elif clase == "CoLiving":
        num_personas = random.randint(3, 6)
        # Mantenemos habitacion individual porque hay regla para ello
        extra_slots = "(habitaciones_individuales SI)"
...
with open(NOMBRE_ARCHIVO_CLP, "w", encoding="utf-8") as f:
    f.write(f";;; INSTANCIAS GENERADAS AUTOMATICAMENTE (V6 LIMPIA)\n\n(definstances instances-generadas\n")
    ...
    for v in viviendas:
        ...
        # Slots específicos (Solo los que sobreviven en V6)
        specific_slots = ""
        # Unifamiliar ya no tiene tamano_jardin en V6
        # Bloque ya no tiene gastos_comunidad en V6
        if v['tipo'] == "Bajo":
            specific_slots = f"(tiene_rejas {v['rejas']})"
...
plt.title(f"Mapa V6: {NUM_SOLICITANTES} Solicitantes")
plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig(NOMBRE_IMAGEN, dpi=100)
print(f">> ¡HECHO! Generado {NOMBRE_ARCHIVO_CLP} compatible con Ontologia V6 Limpia")